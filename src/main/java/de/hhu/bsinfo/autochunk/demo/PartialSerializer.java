...
public final class PartialSerializer {
        ...
        for (; i < fields.length; i++) {
            ...
            switch (fieldSpec.getFieldType()) {
                ...
                case LENGTH:
                    ...
                    UNSAFE.putInt(p_buffer, BYTE_ARRAY_OFFSET + position, UNSAFE.getInt(tmpObject, SizeUtil.ARRAY_LENGTH_OFFSET));
                    position += Integer.BYTES;
                    bytesLeft -= Integer.BYTES;
                    break;
    ...
    static int serializeInterrupted(final Operation p_operation, final byte[] p_buffer, final int p_offset, final int p_length) {
        ...
        switch (fieldSpec.getFieldType()) {
            ...
            case BOOLEAN:
                for (; fieldProcessed < byteCap; fieldProcessed++, position++, fieldLeft--) {
                    UNSAFE.putByte(p_buffer, BYTE_ARRAY_OFFSET + position, UNSAFE.getBoolean(target, fieldSpec.getOffset() + fieldProcessed) ? TRUE : FALSE);
                }
                break;

            case LENGTH:
                for (; fieldProcessed < byteCap; fieldProcessed++, position++, fieldLeft--) {
                    UNSAFE.putByte(p_buffer, BYTE_ARRAY_OFFSET + position, UNSAFE.getByte(target, SizeUtil.ARRAY_LENGTH_OFFSET + fieldProcessed));
                }
                break;
