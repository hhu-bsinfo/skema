...
public class SkemaBenchmark {
    ...
    public static class SkemaState {
        ...
        public void setup() {
            ...
            dataClass = data.getClass();
            m_schema = SchemaRegistry.getSchema(dataClass);

            // Create buffers for serialization/deserialization
            buffer = new byte[m_schema.getSize(data)];
    ...
    public int serializeSkemaFullOffHeap(SkemaState p_state) {
        return Skema.serialize(p_state.data, p_state.offHeapAddress);
    }

    @Benchmark
    public byte[] serializeSkemaPartialOffHeap(SkemaState p_state) {
        p_state.operation.rewind();
        for (int i = 0; i < p_state.buffer.length; i++) {
